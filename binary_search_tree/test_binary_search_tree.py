...
class BinarySearchTreeTests(unittest.TestCase):
  ...
  def test_insert(self):
    ...
    self.bst.insert(7)
    self.bst.insert(6)
    self.assertEqual(self.bst.root.left.right.value, 3)
  ...
  def test_print_traversals(self):
    ...
    self.bst.in_order_print(self.bst.root)
    output = sys.stdout.getvalue()
    self.assertEqual(output, "1\n2\n3\n4\n5\n6\n7\n8\n")

    sys.stdout = io.StringIO()
    self.bst.bft_print(self.bst.root)
    output = sys.stdout.getvalue()
    self.assertEqual(output, "1\n8\n5\n3\n7\n2\n4\n6\n")

    sys.stdout = io.StringIO()
    self.bst.dft_print(self.bst.root)
    output = sys.stdout.getvalue()
    self.assertEqual(output, "1\n8\n5\n7\n6\n3\n4\n2\n")

    sys.stdout = io.StringIO()
    self.bst.pre_order_dft(self.bst.root)
    output = sys.stdout.getvalue()
    self.assertEqual(output, "1\n8\n5\n3\n2\n4\n7\n6\n")

    sys.stdout = io.StringIO()
    self.bst.post_order_dft(self.bst.root)
    output = sys.stdout.getvalue()
